...
async def predict_image(file: UploadFile = File(...)) -> Dict[str, Any]:
    ...
    try:
        ...
        if not file.content_type or not file.content_type.startswith('image/'):
            raise HTTPException(
                status_code=400,
                detail="Nur Bilddateien sind erlaubt (JPEG, PNG, etc.)"
            )
        ...
        logger.info(f"Bild klassifiziert als {prediction} mit Konfidenz {confidence:.3f} in {processing_time:.3f}s")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Fehler bei der Bildverarbeitung: {str(e)}"
        )
